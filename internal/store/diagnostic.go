...
// ─── Upgrade reason codes shared across store and diagnostic layers ───────────
...
// SessionProjectReclassification describes a single session-level project move.
...
// SessionProjectReclassificationCounts reports how many rows would be (or were) affected.
...
// SessionProjectReclassificationResult is returned by ApplySessionProjectReclassification.
...
func (s *SQLiteStore) ListDiagnosticSessions(project string) ([]DiagnosticSessionEvidence, error) {
...
func (s *SQLiteStore) ListPendingProjectMutations(project string) ([]SyncMutation, error) {
	project, _ = NormalizeProject(project)
	project = strings.TrimSpace(project)
...
func (s *SQLiteStore) listPendingProjectMutationsTxLike(q rowQuerier, project string) ([]SyncMutation, error) {
	query := `
...
func (s *SQLiteStore) ReadSQLiteLockSnapshot(ctx context.Context) (SQLiteLockSnapshot, error) {
...
// EstimateSessionProjectReclassification counts how many rows would change
// without mutating anything.
func (s *SQLiteStore) EstimateSessionProjectReclassification(actions []SessionProjectReclassification) (SessionProjectReclassificationCounts, error) {
...
// ApplySessionProjectReclassification backs up the SQLite database then
// reclassifies sessions, observations, and user_prompts atomically.
func (s *SQLiteStore) ApplySessionProjectReclassification(actions []SessionProjectReclassification) (SessionProjectReclassificationResult, error) {
...
// BackupSQLite creates a backup of the SQLite database in the backups subdirectory.
func (s *SQLiteStore) BackupSQLite() (string, error) {
...
// normalizeSessionProjectReclassificationActions deduplicates and validates
// a reclassification action list.
