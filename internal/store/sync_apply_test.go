...
func applyRelationMutation(t *testing.T, s *SQLiteStore, m SyncMutation) error {
	t.Helper()
...
// given sync_id.
func countRelationRows(t *testing.T, s *SQLiteStore, syncID string) int {
	t.Helper()
	var n int
...
// given sync_id.
func countDeferredRows(t *testing.T, s *SQLiteStore, syncID string) int {
	t.Helper()
	var n int
	if err := s.DB().QueryRow(
...
func setupSyncApplyStore(t *testing.T) (s *SQLiteStore, syncObsA, syncObsB string) {
	t.Helper()
	s = newTestStore(t)
...
func insertDeferredRow(t *testing.T, s *SQLiteStore, syncID, entity, payload string, retryCount int, applyStatus string) {
	t.Helper()
	if _, err := s.DB().Exec(`
...
func getDeferredRow(t *testing.T, s *SQLiteStore, syncID string) (applyStatus string, retryCount int) {
	t.Helper()
	if err := s.DB().QueryRow(
