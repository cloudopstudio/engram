...
// database.  Returns a *SQLiteStore ready for assertion.
//
...
func newTestStoreWithLegacySchema(t *testing.T, fixtureRows []legacyObsRow) *SQLiteStore {
	t.Helper()
	...
	// New() returns the Store interface in this fork; these migration tests need
	// the concrete SQLite implementation.
	sqliteStore, ok := s.(*SQLiteStore)
	if !ok {
		t.Fatalf("newTestStoreWithLegacySchema: expected *SQLiteStore, got %T", s)
	}
	return sqliteStore
}
...
func newTestStoreWithLegacySchemaPostP1(t *testing.T, obsRows []legacyObsRow, relRows []legacyRelationRow) *SQLiteStore {
	t.Helper()
...
func newTestStoreWithLegacySchemaPostP2(t *testing.T, relRows []legacyRelationRow) *SQLiteStore {
	t.Helper()
