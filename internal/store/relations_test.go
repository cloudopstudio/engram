...
func setupRelationsStore(t *testing.T) *SQLiteStore {
	t.Helper()
	s := newTestStore(t)
...
func addTestObs(t *testing.T, s *SQLiteStore, title, obsType, project, scope string) (int64, string) {
	t.Helper()
...
func setupEnrolledStore(t *testing.T) *SQLiteStore {
	t.Helper()
	s := setupRelationsStore(t)
...
func addEnrolledObs(t *testing.T, s *SQLiteStore, title string) (int64, string) {
	t.Helper()
...
func countRelationMutations(t *testing.T, s *SQLiteStore, entity, project string) int {
	t.Helper()
	var n int
...
func addTestObsSession(t *testing.T, s *SQLiteStore, sessionID, title, obsType, project, scope string) (int64, string) {
	t.Helper()
	id, err := s.AddObservation(AddObservationParams{
		SessionID: sessionID,
		Type:      obsType,
...
func setupTwoProjectStore(t *testing.T) (s *SQLiteStore, alphaSync1, alphaSync2, betaSync1 string) {
	t.Helper()
	s = newTestStore(t)
...
func insertRelationWithStatus(t *testing.T, s *SQLiteStore, srcSyncID, tgtSyncID, judgmentStatus string) string {
	t.Helper()
	relSyncID := newSyncID("rel")
	if _, err := s.DB().Exec(`
		INSERT INTO memory_relations
...
func setupScanStore(t *testing.T) (s *SQLiteStore, sync1, sync2 string) {
	t.Helper()
	s = newTestStore(t)
...
func setupExportRelationsStore(t *testing.T) (s *SQLiteStore, relAlpha, relBeta string) {
	t.Helper()
	s = newTestStore(t)
