...
func setupBackfillStore(t *testing.T) (s *SQLiteStore, srcSyncID, tgtSyncID string) {
	t.Helper()
	s = newTestStore(t)
...
func insertRelationDirect(t *testing.T, s *SQLiteStore, syncID, sourceID, targetID, judgmentStatus string) {
	t.Helper()
	if _, err := s.DB().Exec(`
...
func insertJudgedRelationDirect(t *testing.T, s *SQLiteStore, syncID, sourceID, targetID string) {
	t.Helper()
	if _, err := s.DB().Exec(`
		INSERT INTO memory_relations
...
func countRelationSyncMutationsByKey(t *testing.T, s *SQLiteStore, relSyncID string) int {
	t.Helper()
	var n int
