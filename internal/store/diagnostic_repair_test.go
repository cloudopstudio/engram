...
func seedRepairRows(t *testing.T, s *SQLiteStore, sessionID, project string) {
	t.Helper()
...
func assertRepairProjects(t *testing.T, s *SQLiteStore, sessionID, sessionProject, observationProject, promptProject string) {
	t.Helper()
...
func scalarString(t *testing.T, s *SQLiteStore, query string, args ...any) string {
	t.Helper()
	var got string
...
func scalarInt(t *testing.T, s *SQLiteStore, query string, args ...any) int {
	t.Helper()
	var got int
	if err := s.DB().QueryRow(query, args...).Scan(&got); err != nil {
		t.Fatalf("query %q: %v", query, err)
