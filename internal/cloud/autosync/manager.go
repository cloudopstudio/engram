...
// ─── Transport types ─────────────────────────────────────────────────────────
// These mirror internal/cloud/mutations.MutationEntry / PushMutationsResult /
// PullMutationsResponse to avoid a circular import between autosync and the
// cloud package itself.

// MutationEntry is a single local mutation to be pushed to the cloud.
...
// PushMutationsResult is the cloud server's response to a batch push.
type PushMutationsResult struct {
	AcceptedSeqs []int64 `json:"accepted_seqs"`
}

// PulledMutation is a single mutation pulled from the cloud server.
...
// PullMutationsResponse is the cloud server's response to a pull request.
...
// CloudTransport is the subset of mutation transport methods the manager needs.
...
// Allows Manager to detect 401 (auth_required) vs 403 (policy_forbidden) vs generic
// transport failures without importing the cloud transport package.
...
// 401 → "auth_required", 403 → "policy_forbidden", otherwise "transport_failed".
...
func (m *Manager) pull(ctx context.Context) error {
	...
	for {
		...
		for _, rm := range resp.Mutations {
			...
			// Phase E: per-entity error policy.
...
// Allows specific reason codes (auth_required, policy_forbidden) to surface
...
// Formula: min(base * 2^(failures-1), maxBackoff) ± jitter.
func (m *Manager) computeBackoff(failures int) time.Duration {
	...
	// Floor at BaseBackoff/2 to avoid extremely short intervals.
