// Package chunkcodec provides chunk ID generation and project canonicalization
// for cloud sync. It is ported from the upstream Gentleman-Programming/engram
// cloud module and adapted to the local store types.
package chunkcodec
...
// ChunkID returns the first 8 hex chars of the SHA-256 of payload.
// This is the content-addressed identifier used in manifest entries.
func ChunkID(payload []byte) string {
	hash := sha256.Sum256(payload)
	return hex.EncodeToString(hash[:])[:8]
}

// CanonicalizeForProject rewrites every session, observation, prompt, and
// mutation inside the raw chunk JSON so that their "project" field is set to
// the given project name. Returns the re-encoded JSON.
...
// ─── payload structs (private, used for mutation canonicalization) ─────────────
...
// DecodeSyncMutationPayload decodes a mutation payload that may be either a
// raw JSON object or a JSON-encoded string containing a JSON object.
