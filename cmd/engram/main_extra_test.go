...
func TestResolveCloudRuntimeConfigIgnoresPersistedToken(t *testing.T) {
	// This fork diverges from upstream issue #343 on purpose: runtime auth comes
	// from the environment only, so a token in cloud.json is never adopted. The
	// server URL is still read from the file.
	cfg := testConfig(t)
	...
	if runtimeCfg.Token != "" {
		t.Fatalf("persisted cloud.json token must be ignored, got %q", runtimeCfg.Token)
	}
